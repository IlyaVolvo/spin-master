...
export class PreliminaryWithFinalPlayoffPlugin extends BaseCompoundTournamentPlugin {
  ...
  protected async handleFinalPhaseLogic(
    ...
    // If all preliminaries are done and no final exists yet, create the final playoff
    if (allPreliminariesComplete && !finalTournament) {
      ...
      return {
        ...
        finalTournamentConfig: {
          type: 'PLAYOFF',
          // Configuration for selecting top players from each group
        },
        message: 'All preliminary rounds completed. Ready to create final playoff.',
      };
    }
