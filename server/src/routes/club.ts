...
/**
 * POST /api/club/cron/lesson-reminders — optional individual-lesson reminder emails
 */
router.post('/cron/lesson-reminders', async (req: Request, res: Response) => {
  try {
    const cronSecret = process.env.CLUB_CRON_SECRET;
    const providedSecret = req.headers['x-club-cron-secret'];
    if (cronSecret && providedSecret !== cronSecret) {
    ...
    const { processLessonReminders } = await import('../services/lessonReminderService');
    const result = await processLessonReminders();
    res.json(result);
  } catch (error) {
    logger.error('Error sending lesson reminders', {
      error: error instanceof Error ? error.message : String(error),
    });
    res.status(500).json({ error: 'Internal server error' });
  }
