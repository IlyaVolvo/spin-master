...
export const PreliminaryWithFinalRoundRobinPostSelectionFlow: React.FC<PostSelectionFlowProps> = ({
  selectedPlayerIds,
  members,
  tournamentName,
  setTournamentName,
    ...
    await runBusy(async () => {
      try {
        ...
        if (!tournamentName.trim()) {
          ...
          tournamentData.name = `Preliminary + Final RR ${dateStr}`;
        } else {
        ...
        if (finalizingPreregistrationId) {
          ...
          onSuccess(buildFinalizeSuccessMessage('Preliminary + Final Round Robin tournament created from preregistration successfully', response.data));
          ...
          <div style={{
            ...
            <div style={{ fontSize: '13px', color: '#333', lineHeight: '1.6' }}>
              ...
              {autoQualifiedCount > 0 && (
                <div><strong>Auto-qualified:</strong> {autoQualifiedCount} (highest rated → skip to final)</div>
              )}
              ...
              <div><strong>Final Round Robin:</strong> {finalRoundRobinSize} players</div>
              {finalRoundRobinSize > minFinalSize && (
                <div style={{ color: '#27ae60' }}>
                  <strong>Extra slots from 2nd/3rd place:</strong> {finalRoundRobinSize - minFinalSize}
                </div>
              )}
            </div>
          </div>
        ...
        {autoQualifiedMemberIds.length > 0 && (
          <div style={{
            ...
            <h5 style={{ margin: '0 0 10px 0', color: '#f39c12' }}>
              Auto-Qualified ({autoQualifiedMemberIds.length} players → Final directly)
            </h5>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '15px', padding: '15px', backgroundColor: '#f9f9f9', borderRadius: '4px' }}>
            <div style={{ marginBottom: '8px' }}>
              <strong>Type:</strong> Preliminary Round Robins + Final Round Robin
            </div>
            <div style={{ marginBottom: '8px' }}>
            ...
            {autoQualifiedCount > 0 && (
              <div style={{ marginBottom: '8px' }}>
                <strong>Auto-Qualified:</strong> {autoQualifiedCount} players (skip to final)
              </div>
            )}
            <div style={{ marginBottom: '8px' }}>
